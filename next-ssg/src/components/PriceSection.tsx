...
const memorialItems: PriceItem[] = [
  { serviceName: "蹄鉄盾", price: "20,000〜" },
  { serviceName: "勝利写真", price: "20,000〜" },
  { serviceName: (<><span>トレーディングカード</span><br /><span className="text-sm">(一種)</span></>), price: "10,000〜" },
  { serviceName: "記念冊子", price: "20,000〜" }
];

const apparelItems: PriceItem[] = [
...
const designItems: PriceItem[] = [
  { serviceName: "馬名/厩舎/牧場ロゴ", price: "30,000〜" },
  { serviceName: "名刺", price: "8,000〜" }
];
...
export const PriceSection: React.FC = () => {
  ...
  const cardRefs = [useFadeInOnScroll<HTMLLIElement>(), useFadeInOnScroll<HTMLLIElement>(), useFadeInOnScroll<HTMLLIElement>()];
          ...
          {priceCards.map((card, index) => (
            ...
            >
              <FeatureCard
                title={card.title}
                items={card.items}
                renderItem={(item: PriceItem, index: number, total: number) => (
                  <li
                    key={index}
